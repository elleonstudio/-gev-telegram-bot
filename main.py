...
# Исправленная инструкция: ЗАПРЕТ на слово "None"
SYSTEM_MSG_NAMING = (
    "Ты эксперт по неймингу файлов. Создай имя файла по тексту с этикетки.\n"
    ...
    "ПРАВИЛА:\n"
    "1. Если китайских иероглифов нет, ПРОПУСТИ ИХ (начни сразу с EnglishName).\n"
    "2. Если размера нет, ПРОПУСТИ ЕГО.\n"
    "3. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать слова 'None', 'Null' или 'БезРазмера'. Просто соединяй нижним подчеркиванием то, что есть.\n"
    "Пример без иероглифов и размера: CastIronPan_747232933_2048244245878.pdf\n"
    "Выведи ТОЛЬКО имя файла, без лишних слов."
)
...
# НОВАЯ УМНАЯ ФУНКЦИЯ ПОИСКА АРТИКУЛА
def find_article(text: str) -> str:
    # Ищет буквы, цифры, пробелы и тире (например: wb 22 tangle teezer)
    match = re.search(r'(?:Артикул|Article)[:\s]*([a-zA-Z0-9\-\_]+(?:[ \t]+[a-zA-Z0-9\-\_]+)*)', text, re.IGNORECASE)
    if match: return match.group(1).strip()
    
    # Резервный поиск всего, что идет до конца строки
    match = re.search(r'(?:Артикул|Article)[:\s]*([^\n]+)', text, re.IGNORECASE)
    if match: return match.group(1).strip()
    return ""
