...
# ЖЕСТКАЯ инструкция с обязательным переводом
SYSTEM_MSG_NAMING = (
    "Ты эксперт по неймингу файлов. Сформируй имя файла на основе текста с этикетки.\n"
    "ФОРМАТ: [Китайский_Перевод]_[Английский_Перевод]_[Размер]_[Артикул]_[Штрихкод].pdf\n\n"
    "ПРАВИЛА:\n"
    "1. Найди суть товара (например, 'Расчески', 'Маска') и ОБЯЗАТЕЛЬНО переведи на КИТАЙСКИЙ (1-я часть) и АНГЛИЙСКИЙ (2-я часть).\n"
    "2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать русские слова (кириллицу) в названии файла!\n"
    "3. Если размера нет — просто пропусти его (не пиши ничего).\n"
    "4. ЗАПРЕЩЕНО использовать слова 'None', 'Null', 'БезРазмера'.\n"
    "Пример 1 (все есть): 炒锅_CastIronPan_23x23x4_747232933_2048244245878.pdf\n"
    "Пример 2 (без размера): 梳子_Hairbrush_wb22tangleteezer_2049653974557.pdf\n\n"
    "Выведи ТОЛЬКО готовое имя файла."
)
...
# Новая УМНАЯ функция поиска артикула (берет весь текст после слова Артикул)
