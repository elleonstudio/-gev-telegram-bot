...
async def write_to_airtable_dostavka(data: dict):
    ...
    try:
        ...
        return "✅ Доставка по РФ: Данные успешно добавлены в базу!"
    except Exception as e:
...
Если размера нет, ставь '-' в FILENAME. Если цвета/материала нет на этикетке, постарайся определить их по фото (если можешь), иначе не пиши."""
            ...
            wb_link = f" 👉 <a href='https://www.wildberries.ru/search?search={art}'>Wildberries</a>" if art != "-" else ""
            ...
            await msg.edit_text(f"❌ Произошла ошибка при обработке PDF:\n`{e}`\n\nПришлите скриншот этой ошибки разработчику.", parse_mode='HTML')
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    menu_text = (
        ...
        "4️⃣ <b>Просто фото/файл этикетки</b> - создает PDF для склада (Китай_Англ_Размер_Арт_Штрихкод.pdf)\n"
