...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "<b>📂 GS Assistant: Главное меню</b>\n\nВыбери нужную функцию или нажми на Руководство для справки."
    keyboard = [
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'open_guide':
        guide = (
            ...
            "1. <b>/paste</b> — Переупаковка расчетов.\n"
            "2. <b>/1688</b> — Данные поставщика с фото.\n"
            "3. <b>/hs</b> — Поиск ТН ВЭД.\n"
            "4. <b>Штрих-коды</b> — Просто кидай фото этикетки, бот предложит имя файла.\n"
            "5. <b>Airtable</b> — Пересылай блоки AIRTABLE_EXPORT."
        )
        await query.edit_message_text(guide, parse_mode='HTML')
    elif query.data == 'check_airtable':
        await query.edit_message_text(f"📊 <b>Статус Airtable:</b>\n\nБаза: <code>{AIRTABLE_BASE_ID}</code>\nТаблицы: <i>{TABLE_ORDERS}, {TABLE_CARGO}</i>\nСтатус: 🟢 Подключено", parse_mode='HTML')
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if caption.startswith('/1688'):
        res = await ask_kimi("Supplier Info CN/EN Tax ID Address. Code blocks.", image_b64=img_b64, system_msg="1688 Expert.")
        await update.message.reply_text(res, parse_mode='Markdown')
    elif caption.startswith('/hs'):
        res = await ask_kimi(f"HS Code 4/6/10 digits. Info: {caption}", image_b64=img_b64, system_msg="Broker.")
        ...
        # ВОТ ТУТ МЫ ВЕРНУЛИ ШТРИХ-КОДЫ
        msg = await update.message.reply_text("⏳ Сканирую штрих-код...")
        ...
        await msg.edit_text(f"📄 <b>Предлагаемое имя:</b>\n<code>{new_name}</code>\n\n✅ Штрих-код: <code>{barcode}</code>\n✅ Артикул: <code>{art}</code>", parse_mode='HTML')

def main():
    app = Application.builder().token(TELEGRAM_TOKEN).build()
    
    app.add_handler(CommandHandler("start", lambda u, c: u.message.reply_text("🤖 GS Assistant запущен! Нажми /menu.")))
    app.add_handler(CommandHandler("menu", show_menu))
    app.add_handler(CallbackQueryHandler(button_callback))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
    app.add_handler(MessageHandler(filters.PHOTO, handle_photo))
    app.add_handler(MessageHandler(filters.Document.ALL, handle_photo)) # PDF как фото обрабатываем
    
    async def set_cmds(application):
        await application.bot.set_my_commands([BotCommand("menu", "Открыть меню функций"), BotCommand("start", "Запуск")])
