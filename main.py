...
import pytesseract
from pyzbar.pyzbar import decodeimport os
import logging
import base64
import re
import aiohttp
...
# Токены (обязательно подставьте свои или настройте Environment Variables)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Логика скачивания (фото или документ)
    ...
    # Конвертация PDF в изображение для анализа
    ...
    # Логика ЭТИКЕТКИ (Авто-переименование)
    ...
    raw_res = await ask_kimi(prompt_label, image_b64=img_b64, system_msg="Ты логист в Китае. Обязательно используй иероглифы.")
    ...
    # Всегда отправляем результат как PDF
    pdf_out = BytesIO()
...
def main():
    ...
    app.add_handler(CommandHandler("menu", lambda u, c: u.message.reply_text("<b>📂 GS Orders Bot:</b>\n\n1️⃣ /paste\n2️⃣ /1688\n3️⃣ /hs\n4️⃣ Фото/PDF этикетки", parse_mode='HTML')))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
    # Теперь слушаем и фото, и документы (PDF)
